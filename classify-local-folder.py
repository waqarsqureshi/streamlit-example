...
def classify():
    folder_path = st.text_input("Enter the path to the folder of images:")
    ...
    if folder_path:
        if os.path.isdir(folder_path):
            ...
            if images:
                ...
                st.image(image_with_classes, caption=images[image_index])
    ...
    if folder_path:
        if os.path.isdir(folder_path):
            ...
            if images:
                ...
                # Display results
                st.image(cam_image, caption=image_path, use_column_width=True)
            else:
                st.warning("No images found in the folder.")
        else:
            st.error("The path is not a valid directory.")
...
if __name__ == "__main__":
   # Display the logo
    ...
    # Create a selectbox in the sidebar to choose between examples
    selected_option = st.sidebar.selectbox("Choose an example:", options)

    left_column, right_column = st.columns(2)

    # Display the first logo in the left column
    logo_path1 = "/home/pms/Pictures//TU Dublin Logo_resized.jpg"  # Update this with the path to your first logo file
    left_column.image(logo_path1, width=100)  # You can adjust the width as needed

    # Display the second logo in the right column
    logo_path2 = "/home/pms/Pictures//pms.png"  # Update this with the path to your second logo file
    right_column.image(logo_path2, width=100)  # You can adjust the width as needed
    st.title("Pavement Surface Condition Index")
    if selected_option == "Image Classification":
        st.title("Image Classification")
        classify()  # Call the main function for Example 1

    elif selected_option == "GRAD-CAM":
        st.title("Welcome to GRAD-CAM")
        gradCam()  # Call the main function for Example 2 (you can modify this as needed)
